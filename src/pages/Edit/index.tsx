...
const Edit = () => {
  ...
  const [maxParticipants, setMaxParticipants] = useState(targetStudy?.infos.find(({ type }) => type === 'limit')?.content);
  const [showingDistrict] = useState(targetStudy?.infos.find(({ type }) => type === 'district')?.content || '');
  const [showingStudyWay] = useState(targetStudy?.infos.find(({ type }) => type === 'studyWay')?.content || '');
  ...
  const handleClickSavePosting = () => {
    type dataKeyType = 'title' | 'content' | 'startDate' | 'endDate' | 'maxStudyMemberCount' | 'district' | 'progressOfStudy' | 'tags';
    ...
    const putStudy = async (data: dataType) => {
      ...
      try {
        const response = await axios.put(`${process.env.END_POINT}api/studies/${id}`, data, { headers });
        if (response.status === 404) {
          console.log('404 error');
          return;
        }
      ...
      <S.Container>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <div>
            <S.SelectsContainer>
              <S.Label>지역</S.Label>
              <DropDown width="77px" height="48px" type="district" selectTitle={districts[0].content || showingDistrict} options={districts} />
              ...
              <DropDown width="189px" height="48px" type="studyWay" selectTitle={selectTitles[0] || showingStudyWay} options={options} />
            </S.SelectsContainer>
          </div>
          <div>
